...
export const Login = () => {
    ...
    const facebookCallback = (response: FacebookResponse) => {
        ...
        if (response?.accessToken) {
            dispatch(profileSlice.actions.loginWithFacebook(
                // @ts-ignore 
                { name: response.name, email: response.email, picture: response.picture.data.url }
            ))
            // @ts-ignore 
    ...
    return (
        <>
            ...
            <FacebookLogin
                ...
                icon="fa-facebook"
                containerStyle={
                    { display: 'block', width: window.innerWidth > 680 ? '45%' : '80%' }
                }
            />
            {error && (
